...
def load_schablone(path: pathlib.Path) -> pd.DataFrame:
    ...
    text_cols = [
        "Customercode","ItemCode","DescriptionGerman",
        "DescriptionItalian","UnitofMeasurement"
    ]
    for col in text_cols:
        if col in df.columns:
            df[col] = df[col].fillna("").astype(str).str.strip()

    numeric_cols = ["PCPerUnit","SumQty","SumQtyBox","LastorderedQty"]
    for col in numeric_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

    if "LastDocDatum" in df.columns:
        df["LastDocDatum"] = pd.to_datetime(df["LastDocDatum"], errors="coerce")
        df["DaysSinceLastOrder"] = (
            pd.Timestamp.today() - df["LastDocDatum"]
        ).dt.days.fillna(9999)

    df["SearchText"] = (
        df.get("DescriptionGerman","").astype(str) + " " +
        df.get("DescriptionItalian","").astype(str)
    )
